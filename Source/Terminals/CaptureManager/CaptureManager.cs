...
namespace Terminals.CaptureManager
{
    internal static class CaptureManager
    {
        ...
        {
            var name = ResolveFileName(favorite);
            var formatedDate = DateTime.Now.ToString("yyyy_MM_dd_hh_mm_ss");
            var filename = $"{name}{formatedDate}.png";
            var tempFile = Path.Combine(CaptureRoot, filename);
